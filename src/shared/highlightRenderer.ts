import type { PhotoHighlight } from './types';
...
export function buildHighlightSvg(width: number, height: number, highlight: PhotoHighlight) {
  ...
  return [
    ...
    `<circle cx="${circle.x}" cy="${circle.y}" r="${circle.radius}" fill="none" stroke="#ff0000" stroke-width="${strokeWidth}" stroke-dasharray="${dash} ${gap}" stroke-linecap="round"/>`,
    '</svg>'
  ].join('');
}
