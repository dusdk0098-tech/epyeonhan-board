...
async function main() {
  ...
  assert(!app.includes('사진에 보드판 삽입') && !app.includes('보드판 입력값 자동 적용'), 'photo ledger mode must not expose board insertion or board-field sync options');
  assert(app.includes('사진대지 PDF는 보드판을 삽입하지 않고 사진, 하단정보, 강조 효과만 정리합니다.'), 'photo ledger mode must explain the board-free PDF scope');
  ...
  assert(app.includes('사진별 하단정보, 강조 효과와 출력 순서를 PDF에 반영합니다.'), 'photo ledger UI must explain per-photo output and highlight');
  assert(app.includes('moveSelectedPhotoOrder') && app.includes('출력 순서'), 'photo ledger UI must support per-photo output order');
  assert(!app.includes('보드 내용 불러오기') && app.includes('체크 사진에 적용'), 'photo ledger per-photo helper actions must avoid board-field shortcuts');
  assert(app.includes('showBoard: false') && app.includes('photoLedgerUseBoardFields: false'), 'photo ledger preview and generation must force board overlay and board-field sync off');
