...
PEDIT는 데스크톱 Electron 앱이므로 창 크기, 패널 비율, 버튼 텍스트, 아이콘, 미리보기 영역, 인쇄/저장 액션의 시각적 안정성이 중요하다.
...
Electron 전용 화면은 기본 창 크기, 최대화 상태, 최소 지원 크기를 우선 확인한다. 웹 공개 페이지가 있으면 tablet/mobile도 확인한다.
